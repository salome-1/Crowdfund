...
<div class="container">
    ...
    @endif
     <form method="POST" action="/projects/{{$project->id}}" enctype="multipart/form-data">
       ...
       <div class="form-group row">
          <label for="opened_at" class="col-md-4 col-form-label text-md-right">{{ __('Tanggal dibuat project') }}</label>
           <div class="col-md-6">
          ...
          <label for="description" class="col-md-4 col-form-label text-md-right">{{ __('Deskripsi Project') }}</label>
           <div class="col-md-6">
              <textarea rows="4" cols="50" style="resize:none" id="description" type="text" class="form-control{{ $errors->has('description') ? ' is-invalid' : '' }}" name="description"
                  value="{{ old('description') ? old('description') : $project->description }}" required></textarea>
       ...
       <div class="form-group row">
          <label for="slot" class="col-md-4 col-form-label text-md-right">{{ __('Submit Gambar') }}</label>
           <div class="col-md-6">
              <input id="project_image" type="file" class="form-control{{ $errors->has('project_image') ? ' is-invalid' : '' }}" name="project_image"
                value="{{ old('project_image') ? old('project_image') : $project->project_image }}" required>
